...
export default function ArticleEntry({ article }: { article: ArticleType }) {
  return (
    <Link
      ...
      className="group flex gap-4 py-3 px-2 h-40 hover:bg-gray-50"
    >
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        <h3 className="text-orange-400">{article.section}</h3>
        <h2 className="italic text-2xl font-bold line-clamp-1 group-hover:underline group-hover:text-orange-400 group-active:text-orange-400">
          {article.title}
        </h2>
